...
module.exports = {
    ...
    execute: function (client, message, args) {
        if (args.length === 0) {
            ...
            conn.query("SELECT `m`.`content`,`a`.`url` AS `attachmentURL` FROM `messages` `m` LEFT JOIN `attachments` `a` ON `m`.`id`=`a`.`message_id` WHERE `m`.`channel` = ? AND `m`.`timestamp` >= ? AND `m`.`timestamp` <= ? ORDER BY `m`.`timestamp` DESC", [message.channel.id, timestamp, end_timestamp], async (error, result, fields) => {
                if (error) throw error;
